...
test('computes pools and indicative odds; the underdog shows the higher odd', () => {
  const pool = [
    new Bet({ marketId: 'm1', bettorId: 'u1', selectionId: 'A', stake: 7000 }),
    new Bet({ marketId: 'm1', bettorId: 'u2', selectionId: 'B', stake: 3000 }),
  ]

  const odds = OddsCalculator.calculate('m1', pool, 0)
  expect(odds.totalPool).toBe(10000)

  const a = odds.entries.find((entry) => entry.selectionId === 'A')!
  const b = odds.entries.find((entry) => entry.selectionId === 'B')!
