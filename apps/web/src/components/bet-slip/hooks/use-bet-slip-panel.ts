...
import type { SlipMode } from '../data/modes'
...
export function useBetSlipPanel() {
  ...
  const oddsQueries = useQueries({
    queries: selections.map((selection) => ({
      ...
      queryFn: async (): Promise<MarketOddsDTO> =>
        (await api.get<MarketOddsDTO>(oddsUrl(selection))).data,
    })),
  })
  ...
  const placeSingles = useMutation({
    ...
    onSuccess: (results) => {
      ...
      if (placed.length) {
        notify.success(
          placed.length === 1 ? 'Aposta confirmada.' : `${placed.length} apostas confirmadas.`,
        )
      }
      // The rejected ones stay in the slip so they can be fixed and retried.
      if (failed.length)
        notify.failure(failed[0].reason, 'Não foi possível confirmar parte das apostas.')
    },
  ...
  // Panel chrome: collapsed/expanded, and the portal gate (the slip renders
  // into document.body, so the first server render has to bail out).
